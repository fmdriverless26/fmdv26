...
class MPC:
    ...
    def _build_solver(self):
        ...
        for k in range(self.N + 1):
            ...
            if k < self.N:
                ...
                # Terminal step uses last reference
                rx = P[4 + 4*(self.N-1)]
                ry = P[5 + 4*(self.N-1)]
                ryaw = P[6 + 4*(self.N-1)]
                rv = P[7 + 4*(self.N-1)]
            ...
            cte_weight = self.w_cte if k < self.N else 2.0 * self.w_cte

            cost += self.w_yaw * yaw_error**2
            cost += self.w_speed * v_err**2
            cost += cte_weight * cte**2

            # Controls exist only for k < N
            if k < self.N:
    ...
    def solve(self, state, ref):
        ...
        # Making sure each horizon step has all necessary elements
        if len(ref) != 4 * self.N:
            print(f"Error: ref has {len(ref)} elements, expected {4 * self.N}")
            return self.last_steer, 0.0
        
        # Combine state and reference into parameter vector
        P_val = state + ref
        
        # Initial guess
       
        x0 = []
        #future state equal to current state
        for _ in range(self.N + 1):
                x0 += state[0:4]

        #steering equal to previous steering, accel equal to 0
        for _ in range(self.N):
...
class Controller(Node):
    ...
    def loop(self):
        try:
            ...
            msg.drive.steering_angle = float(steer) * 0.85
